#!/usr/bin/env python3
"""S3 spike — Playwright capture of the v3 walk: forces the v3 model,
clicks Walk, records a 360° orbit GIF of Lily walking in place plus
front/side stills.

Writes into spike3/shots3/:
  walk-orbit3.gif  walk-front3.png  walk-side3.png

Run (server must be up on :8123):
  /tmp/kilo/venv/bin/python spike3/shots3/make_walk_gif3.py
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        assert pg.evaluate("window.__spike3.getModelVersion()") == "v3", \
            "expected v3 default"
        pg.wait_for_function("window.__spike3.whenWalkLoaded('v3').then(()=>true)",
                             timeout=30000)
        ...
        shot("walk-front3.png", 0)
        shot("walk-side3.png", 90)

        print("walk orbit gif (v3)")
        N = 36
        frames = []
        for i in range(N):
            deg = i * (360.0 / N)
            ...
            vp.screenshot(path="/tmp/kilo/_walk3_frame.png")
            im = Image.open("/tmp/kilo/_walk3_frame.png")
            w, h = im.size
            nw = 280
            nh = int(h * nw / w)
            frames.append(im.resize((nw, nh), Image.LANCZOS))
        frames[0].save(
            os.path.join(SHOT, "walk-orbit3.gif"),
            save_all=True, append_images=frames[1:],
            duration=90, loop=0, optimize=True,
        )
        print("  saved walk-orbit3.gif")
    ...
    for f in ("walk-orbit3.gif", "walk-front3.png", "walk-side3.png"):
    ...
    if console_errors or bad:
        ...
        print("\nRESULT: OK (v3 walk active, offline, no console errors)")
