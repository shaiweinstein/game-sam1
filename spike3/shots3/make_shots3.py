#!/usr/bin/env python3
"""S3 spike — Playwright verification of Lily v3: stills, 360° orbit GIF,
console-error + offline request-log check, and fps.

Same flow as shots2/make_shots2.py but for the v3 model (v3 is the viewer
default; asserted). Body shots use zoom 2.55 so the whole kid fits with
headroom (the v3 crown sits at z 0.99).

Writes into spike3/shots3/:
  front.png back.png left.png right.png  45.png 135.png 225.png
  face-closeup.png  with-2d-panel.png  orbit3.gif

Run (server must be up on :8123):
  /tmp/kilo/venv/bin/python spike3/shots3/make_shots3.py
"""
...
SHOT = os.path.join(ROOT, "spike3", "shots3")
URL = "http://localhost:8123/spike3/index.html"
VIEWPORT = {"width": 1280, "height": 800}
FIT = 2.55          # radius that frames the full v3 body in this viewport
...
def main():
    ...
    with sync_playwright() as p:
        ...
        assert pg.evaluate("window.__spike3.getModelVersion()") == "v3", \
            "expected v3 to be the default active model"
        ...
        shot("face-closeup.png", deg=0, zoom=1.55)
        shot("with-2d-panel.png", deg=0, zoom=FIT)
        ...
        for i in range(N):
            ...
            vp.screenshot(path="/tmp/kilo/_orbit3_frame.png")
            im = Image.open("/tmp/kilo/_orbit3_frame.png")
            w, h = im.size
            nw = 280
            nh = int(h * nw / w)
            frames.append(im.resize((nw, nh), Image.LANCZOS))
        frames[0].save(
            os.path.join(SHOT, "orbit3.gif"),
            save_all=True, append_images=frames[1:],
            duration=90, loop=0, optimize=True,
        )
        print("  saved orbit3.gif")
    ...
    gif_kb = os.path.getsize(os.path.join(SHOT, "orbit3.gif")) // 1024
    if console_errors or bad or fps < 55:
        print("\nRESULT: NEEDS ATTENTION")
    else:
        print("\nRESULT: OK (offline, no console errors, ~60fps)"
              + ("" if gif_kb <= 2048 else "  [orbit3.gif > 2MB!]"))
