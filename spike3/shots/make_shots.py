#!/usr/bin/env python3
"""S1 spike — Playwright verification: screenshots, 360° orbit GIF,
...
Writes into spike3/shots/:
  ...
  face-closeup.png  with-2d-panel.png
  orbit.gif

Run (server must be up on :8123):
  /tmp/kilo/venv/bin/python spike3/shots/make_shots.py
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for i in range(N):
            ...
            vp.screenshot(path="/tmp/kilo/_orbit_frame.png")
            im = Image.open("/tmp/kilo/_orbit_frame.png")
            w, h = im.size
            nw = 280
            nh = int(h * nw / w)
            frames.append(im.resize((nw, nh), Image.LANCZOS))
        # assemble GIF (dispose none; full frames for smoothness)
        frames[0].save(
            os.path.join(SHOT, "orbit.gif"),
    ...
    gif_kb = os.path.getsize(os.path.join(SHOT, "orbit.gif")) // 1024
    if console_errors or bad or fps < 55:
        print("\nRESULT: NEEDS ATTENTION")
    else:
        print("\nRESULT: OK (offline, no console errors, ~60fps)"
              + ("" if gif_kb <= 2048 else "  [orbit.gif > 2MB!]"))
