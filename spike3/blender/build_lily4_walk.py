"""Build spike3/assets/lily4_walk.glb — Lily v4 skinned to the Mixamo
"Walking" clip (FBX Binary, 30fps, In Place). Geometry comes from
build_lily4.py (continuous full-coverage one-piece swimsuit — the S4 fix);
the ENTIRE S3 retarget/conform pipeline below is kept exactly as it was:

S4 skinning additions (only what the new suit needs, v3 rules unchanged):
  * SUIT: one watertight loft now. Above the hip it rides Spine->Hips
    exactly like v3's "Suit". Below the hip the ring wraps the thighs, so
    per-vertex we hand that weight to the SAME Hips->*UpLeg blend the old
    SuitTab lofts used (deterministic function of the bind position:
    |x| selects the side + smoothstep ramp below z 0.30 -> 0.242), while
    the front/back crotch-bridge verts (small |x|) stay on Hips/Spine.
  * TRIMHEM: same per-vertex Hips / *UpLeg side rule (v3 had it rigid to
    Hips while its hem cut across the free-standing tabs).
  * The old SuitTab*/TrimTab* bindings are gone (the meshes are gone).

The v3 walk fixes (kept verbatim):
...
accessor vertex counts per primitive: all 32 parts joined).
"""
import bpy
import math
import os
...
OUT = BASE + "/assets/lily4_walk.glb"
...
WRI_L = Vector((0.159, -0.008, 0.318))    # modelled wrist centres (build_lily4.py)
...
# modelled elbow joint centres (build_lily4.py, world): rig shoulder points
...
LEG_BLEND_R = zblend("mixamorig:RightUpLeg", "mixamorig:RightLeg",
                     knee_z + 0.030, knee_z - 0.030)
# (S4: the old TAB_BLEND pair is absorbed into the custom suit_bind rule)

# ---------- S4 custom suit rule ------------------------------------------
# The v4 one-piece is ONE watertight loft: above the hip line it is the
# torso tube (Spine->Hips, same as v3's "Suit"); below, the ring wraps the
# thighs (the meshes the old SuitTab* were) plus crotch bridges front and
# back (pelvis region the old suit had). Per-vertex decision is a purely
# DETERMINISTIC function of the bind position, so re-running the export is
# stable and verifiable:
#   side = which thigh a wrap vertex is on (|x| ramps 0.016 -> 0.032; the
#          bridge columns near the midline stay on the pelvis),
#   t    = ramp 0 at z 0.300 -> 1 at z 0.242, matching the geometry
#          blend in build_lily4.py (HIP_LINE/BLEND_LO).
# t=1 at the hem side wrap => that vertex rides the SAME bone chain the
# old SuitTab hem had (Hips->UpLeg zblend 0.278/0.206 — the ramp below
# mirrors it, so the proven no-tear behaviour of the tabs carries over),
# and t=0 at the crotch bridge keeps the old spine/hip binding.
def _ss(x):
    x = min(1.0, max(0.0, x))
    return x * x * (3 - 2 * x)
...
def suit_bind(co):
    ...
    # pelvis/spine share of the remaining weight == BODY_BLEND's bot():
    if co.z >= 0.50:
        w_sp, w_hp = 1.0, 0.0
    elif co.z <= 0.36:
        w_sp, w_hp = 0.0, 1.0
...
def hem_trim_bind(co):
    # the hem trim ring is entirely at thigh height: side wrap verts follow
    # that thigh, bridge verts (small |x|) follow the pelvis.
...
BINDINGS.update({
    ...
    "Suit":       suit_bind,        # S4 custom: torso+thigh-wrap+crotch rule
    "TrimNeckline": [("mixamorig:Spine", None)],
    "TrimHem":    hem_trim_bind,    # S4 custom: wraps follow the thighs
    "DaisyC":     BODY_BLEND,
})
for k in range(4):
...
for m in meshes:
    ...
    if callable(spec):          # S4 custom multi-bone rule (Suit, TrimHem)
        # collect every bone the rule touches, then weight per vertex
        bones = []
        seen = set()
    ...
    elif spec[0][1] is None:    # rigid part
...
# ---------- S4 PROOF: no suit vertex unbound (skinsim pattern) ----------
for m in meshes:
...
for tag, loc in (("side", (2.0, 0.0, 0.62)), ("front", (0, -2.0, 0.62))):
    ...
    for f in (1, 9, 17, 25):
        sc.frame_set(f)
        bpy.context.view_layer.update()
        sc.render.filepath = f"/tmp/kilo/walk4_{tag}_f{f}.png"
