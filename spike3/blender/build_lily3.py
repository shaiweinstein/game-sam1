#!/usr/bin/env python3
"""S3 spike — 3D Lily v3: hair rebuild, kid shoulders/two-segment arms,
smooth legs + real bare feet. Same face + swimsuit as v2 (both passed).

Run:  blender --background --python spike3/blender/build_lily3.py
Out:  spike3/assets/lily3.glb   (neutral A-pose, no animation)

v3 fixes from the family review of v2 (keep v2 script untouched):
...
OUT = os.path.join(ROOT, "spike3", "assets", "lily3.glb")
...
def main():
    ...
    # daisy on the chest (2D center y190 -> z 0.478). SEATED: petals are
    # embedded 2.5mm into the fabric (they used to float ~1cm off the suit,
    # the "misaligned center" read); only the trim center bumps proud.
    dz = 0.478
    dy = 0.004 - (interp(dz, 2) + 0.008) + 0.0025
    parts.append(ellipsoid("DaisyC", (0.0085, 0.0065, 0.0085),
                           (0, dy - 0.002, dz), M["trim"]))
    for k, (ox, oz) in enumerate(((0, 0.0135), (0.0135, 0),
                                  (0, -0.0135), (-0.0135, 0))):
        parts.append(ellipsoid("DaisyP%d" % k, (0.0098, 0.0065, 0.0098),
                               (ox, dy, dz + oz), M["daisy"]))
...
def debug_renders():
    ...
    try:
        ...
        for name, loc in (("front", (0, -2.3, 0.62)),
            ...
            con.target = tgt
            sc.camera = cam
            sc.render.filepath = os.path.join("/tmp/kilo", "lily3_%s.png" % name)
            bpy.ops.render.render(write_still=True)
            bpy.data.objects.remove(cam, do_unlink=True)
        print("debug renders written to /tmp/kilo/lily3_*.png")
